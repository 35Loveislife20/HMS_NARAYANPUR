...
import {
    FaUsers, FaUserCheck, FaCalendarCheck, FaBed, FaSignOutAlt, FaSyncAlt, FaExclamationTriangle,
} from "react-icons/fa";
...
function Dashboard() {
    ...
    const fetchDashboardStats = async (showLoader = true) => {
        try {
            ...
            const response = await fetch(`${API_URL}/dashboard/stats`, { method: "GET", headers: getHeaders() });
            const data = await response.json();
            if (!response.ok) throw new Error(data.message || "Unable to load stats.");
            if (!data.success || !data.stats) throw new Error("Invalid response.");
            setStats({
    ...
    const fetchRecentAppointments = async (showLoader = true) => {
        try {
            if (showLoader) setAppointmentsLoading(true); setAppointmentsError("");
            const response = await fetch(`${API_URL}/dashboard/recent-appointments?limit=5`, { method: "GET", headers: getHeaders() });
            const data = await response.json();
        ...
        } catch (err) { setAppointmentsError(err.message || "Unable to load appointments."); setAppointments([]); }
        finally { if (showLoader) setAppointmentsLoading(false); }
    };
    ...
    const loadDashboard = async (showLoader = true) => {
        ...
        } finally { if (showLoader) { setLoading(false); setAppointmentsLoading(false); } }
    };
    ...
    const handleLogout = () => { localStorage.removeItem("hms_token"); localStorage.removeItem("hms_user"); navigate("/", { replace: true }); };
        ...
        <main className="dashboard-page">
            <header className="dashboard-topbar">
                ...
                <div className="topbar-right">
                    <button type="button" className={`btn-3d btn-refresh ${refreshing ? "is-refreshing" : ""}`} onClick={handleRefresh} disabled={refreshing}>
                ...
                {statCards.map((stat) => (
                    <div className="stat-card" key={stat.title}>
                        ...
                        <div className="stat-content">
                            ...
                            {loading ? <div className="stat-loading">Loading...</div> : <h3>{stat.value.toLocaleString()}</h3>}
                        </div>
                    </div>
                ))}
            </section>
            ...
            <section className="dashboard-content">
                <div className="dashboard-panel appointments-panel">
                    ...
                    {appointmentsLoading ? (
                        <div className="appointments-loading"><FaSyncAlt className="refresh-spinning" /><span>Loading...</span></div>
                    ) : appointments.length === 0 ? (
                        <div className="appointments-empty"><FaCalendarCheck /><strong>No appointments found</strong><span>Recent appointments will appear here.</span></div>
                    ) : (
                            ...
                            {appointments.map((appointment) => {
                                ...
                                return (
                                    <div className="appointment-row" key={appointment.id}>
                                        ...
                                        <span className={`status ${getStatusClass(appointment.status)}`}>{getStatusText(appointment.status)}</span>
                                    </div>
                                );
                            })}
                        </div>
                ...
                <div className="dashboard-panel heart-panel">
                    <div className="panel-header"><h2>Heart Model</h2></div>
                    <div className="heart-image-container">
                        <video src="/video-from-rawpixel-id-26020147-sd.mp4" autoPlay loop muted playsInline
                            onError={(e) => { e.target.style.display = 'none'; document.getElementById('heart-panel-fallback').style.display = 'block'; }} />
                        <img id="heart-panel-fallback" src="https://images.unsplash.com/photo-1621592479167-0f3db9cb59dd?q=80&w=800&auto=format&fit=crop" alt="Backup" style={{ display: 'none' }} />
                    </div>
                </div>
            </section>
        </main>
